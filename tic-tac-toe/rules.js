...
function begin_play(){
	...
	if(isEmpty($('#player1_id').val()) || isEmpty($('#player2_id').val())){
		//alert and return if there are not 2 players
		return alert("Two player game, both fields are mandatory.");
	}

	//verification and subsequent actions
		...
		//alert if game has already started
		else if(game_started()){
...
function reset_play(){
		...
		//reset started status, table values, and state of the board
		started = false;
...
function play() {
	...
	// Validate the move inputs on board
	var valid = false;
